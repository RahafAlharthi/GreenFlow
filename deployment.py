...
# Load the YOLO model
model = YOLO("C:\\Users\\Ref42\\OneDrive\\سطح المكتب\\Capestone_Project\\YOLO_Model (1).pt")
...
if page == "Visual Content Processing":
    ...
    if upload_file is not None:
        ...
        if os.path.exists(output_video_path):
            ...
            # st.write(f"Total objects detected: {sum(object_counts)}")
...
if page == "SUMO Simulation With Agent":
    ...
    df = pd.read_csv('simulation_data (1)_4.csv')
    ...
    for i, bar in enumerate(bar1):
        # Calculate yval for vehicle counts (for bar1) at the top of the vehicle count bar
    ...
    for i, bar in enumerate(bar2):
        # Calculate yval for average wait time (for bar2) on top of the stacked bars
        yval = bar.get_height() + vehicle_counts[i]  # Base at the height of the vehicle count bar
        ax3.text(bar.get_x() + bar.get_width() / 2, yval, round(bar.get_height(), 2), ha='center', va='bottom')
    ...
    df2 = pd.read_csv('simulation_data (1)_4.csv')
